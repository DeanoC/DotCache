...
This catches the regression where (a) v_tolerance was unsettable from
the CLI, and (b) longbench was missing the §7 knobs entirely.
"""
...
PAPER_BENCHES = ["pg19_perplexity", "niah", "ruler", "longbench"]

# Flags every paper bench MUST expose after Step 0.
REQUIRED_FLAGS = [
    "--v-tolerance",       # paper §7: 0.05; required (no silent default)
    "--use-int4-values",   # paper §3.1/§7
    "--group-size",        # paper §7: 16
    "--tau-cov",           # paper §7: 0.995
    "--k-min",
    "--k-max",
    "--ranking-fallback",
    "--ranking-r",
...
def test_paper_bench_rejects_missing_v_tolerance(bench: str):
    ...
    No silent default — this is the teeth that prevents the Apr 17–24 bug
    from recurring.
    """
    script = REPO / "benchmarks" / "paper" / f"{bench}.py"
