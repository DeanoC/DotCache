...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize Qwen3.5 CUDA Needle pack artifacts.")
    parser.add_argument("input", help="Needle pack JSONL artifact.")
...
def _render(rows: list[dict]) -> str:
    ...
    for key in sorted(grouped):
        ...
        retrieval = [1.0 if row.get("needle_answer_correct") else 0.0 for row in sample_rows]
        exact = [1.0 if row.get("needle_answer_exact_match") else 0.0 for row in sample_rows]
