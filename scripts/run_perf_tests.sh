...
if [[ ! -f "$OUT_DIR/test3_niah_8k.pagein.json" ]]; then
  ...
  DOTCACHE_V_TOL=0.05 python "$ROOT_DIR/benchmarks/paper/niah.py" \
    ...
    --output "$OUT_DIR/test3_niah_8k.json" \
    ...
    --pagein-telemetry
  git add "$OUT_DIR/test3_niah_8k.json" "$OUT_DIR/test3_niah_8k.pagein.json" 2>/dev/null || true
  git commit -m "perf Test 3: NIAH 8K certified + pagein telemetry" || true
  git push origin "$BRANCH" || true
fi

if [[ ! -f "$OUT_DIR/test3_pg19_8k.pagein.json" ]]; then
  ...
  DOTCACHE_V_TOL=0.05 python "$ROOT_DIR/benchmarks/paper/pg19_perplexity.py" \
    ...
    --output "$OUT_DIR/test3_pg19_8k.json" \
    --tau-cov 0.995 --k-min 2 --k-max 128 \
    --ranking-fallback --ranking-r 1 \
    --score-consistency-check --eps-guard 0.01 \
    --exploration-rate 0.02 --rung1-threshold 0.02 --rung1-multiplier 2.0 \
    --pagein-telemetry
  git add "$OUT_DIR/test3_pg19_8k.json" "$OUT_DIR/test3_pg19_8k.pagein.json" 2>/dev/null || true
  git commit -m "perf Test 3: PG-19 8K certified + pagein telemetry" || true
  git push origin "$BRANCH" || true
fi

if [[ ! -f "$OUT_DIR/test3_ruler_8k.pagein.json" ]]; then
  ...
  DOTCACHE_V_TOL=0.05 python "$ROOT_DIR/benchmarks/paper/ruler.py" \
    ...
    --output "$OUT_DIR/test3_ruler_8k.json" \
    --tau-cov 0.995 --k-min 2 --k-max 128 \
    --ranking-fallback --ranking-r 1 \
    --score-consistency-check --eps-guard 0.01 \
    --exploration-rate 0.02 --rung1-threshold 0.02 --rung1-multiplier 2.0 \
    --pagein-telemetry
  git add "$OUT_DIR/test3_ruler_8k.json" "$OUT_DIR/test3_ruler_8k.pagein.json" 2>/dev/null || true
  git commit -m "perf Test 3: RULER 8K certified + pagein telemetry" || true
  git push origin "$BRANCH" || true
fi

# --- Test 1: decode throughput matrix ---
if [[ ! -f "$OUT_DIR/test1_throughput.json" ]]; then
  ...
  python "$ROOT_DIR/benchmarks/bench_throughput_8k.py" \
    ...
    --output "$OUT_DIR/test1_throughput.json"
  git add "$OUT_DIR/test1_throughput.json"
  git commit -m "perf Test 1: throughput matrix (dense/cert/cert-no-fb/quant-only × 10)"
  git push origin "$BRANCH"
fi

# --- Test 2: 7-phase latency breakdown ---
if [[ ! -f "$OUT_DIR/test2_phase_breakdown.json" ]]; then
  ...
  python "$ROOT_DIR/benchmarks/bench_latency_breakdown_8k.py" \
    ...
    --output "$OUT_DIR/test2_phase_breakdown.json"
  git add "$OUT_DIR/test2_phase_breakdown.json"
