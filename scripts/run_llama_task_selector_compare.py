...
from benchmarks.bench_qwen35_attention_subset_dotcache_needle import (  # noqa: E402
    _repeat_trim_ids,
    build_needle_prompt_inputs,
    score_needle_answer,
)
from benchmarks.bench_qwen35_attention_subset_dotcache_serving import _aggregate_record_values  # noqa: E402
from dotcache.config import DotCacheConfig  # noqa: E402
from dotcache.integrations.llama import (  # noqa: E402
    LlamaDotCacheHarness,
    resolve_hf_auth_kwargs,
    transformers_available,
)
from dotcache.selector_profiles import resolve_learned_page_selector_profile  # noqa: E402
...
DEFAULT_SELECTOR_ARTIFACT = (
    ...
    / "benchmarks"
    / "results"
    / "llama32_selector_suite_20260401"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a compact Llama real-task selector comparison suite.")
...
def _build_harness(args: argparse.Namespace, *, profile: str) -> LlamaDotCacheHarness:
    ...
    if profile == "exact":
        ...
        selector_logit_offset = 0.0
        selector_prompt_family = None
        selector_prompt_variant = None
    else:
        resolved_profile = resolve_learned_page_selector_profile(
            profile=profile,
            model_id=args.model_id,
            target_candidate="M3/affine/4/float16",
            logit_offset=0.0,
        )
        selector_path = args.selector_artifact
        selector_profile = resolved_profile.profile
        selector_target_candidate = resolved_profile.target_candidate
        selector_logit_offset = resolved_profile.logit_offset
        selector_prompt_family = args.selector_prompt_family
        selector_prompt_variant = args.selector_prompt_variant
    dotcache_config = DotCacheConfig(
        ...
        learned_page_selector_path=selector_path,
        learned_page_selector_prompt_family=selector_prompt_family,
        learned_page_selector_prompt_variant=selector_prompt_variant,
        learned_page_selector_profile=selector_profile,
        learned_page_selector_scope="KV",
        learned_page_selector_target_candidate=selector_target_candidate,
        learned_page_selector_logit_offset=selector_logit_offset,
    )
    return LlamaDotCacheHarness.from_pretrained(
        args.model_id,
        dotcache_config,
...
def main() -> int:
    ...
    if not transformers_available():
        raise SystemExit("run_llama_task_selector_compare.py requires the optional transformers dependencies")
