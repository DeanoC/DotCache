...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a Qwen3.5 attention-subset page-trace capture sweep and merge the resulting manifests.")
...
def main() -> int:
    ...
    if not transformers_available():
        raise SystemExit("run_qwen35_page_trace_capture_sweep.py requires the optional transformers dependencies")

    prompt_families = args.prompt_family or ["reasoning", "instruction", "retrieval"]
    prompt_lengths = args.prompt_length or [512, 1024]
    decode_steps = args.decode_steps or [4]
        ...
        for prompt_variant, prompt_unit in family_variants.items():
            for prompt_length in prompt_lengths:
                for decode_step_count in decode_steps:
                    run_name = f"family-{prompt_family}_variant-{prompt_variant}_prompt{prompt_length:04d}_decode{decode_step_count:02d}"
    ...
    merged_manifest = merge_page_trace_manifests(
        ...
        source="qwen35_page_trace_capture_sweep",
    )
