...
def build_report(rows: list[dict[str, Any]], trial_rows: list[dict[str, Any]] | None = None) -> tuple[dict[str, Any], str]:
    ...
    for (task_name, prompt_length), grouped in sorted(by_group.items(), key=lambda item: (item[0][0], item[0][1])):
        ...
        quality_decode = float(quality.get("decode_ms_per_step", 0.0) or 0.0)
        systems_decode = float(systems.get("decode_ms_per_step", 0.0) or 0.0)
        ...
        row = {
            ...
            "exact_decode_ms_per_step": float(exact.get("decode_ms_per_step", 0.0) or 0.0),
            ...
            "systems_teacher_forced_logit_max_abs_error": float(
                systems.get("teacher_forced_logit_max_abs_error", 0.0) or 0.0
            ),
        }
