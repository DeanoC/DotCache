...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "benchmarks" / "results" / "qwen35_context_sweep_20260329"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a per-context Qwen3.5 native-vs-TurboQuant sweep.")
    ...
    parser.add_argument("--contexts", type=int, nargs="+", default=[448, 1024, 2048, 4096, 8192, 16384])
    parser.add_argument("--max-new-tokens", type=int, default=4)
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR))
    parser.add_argument("--layer-profile", default="configs/layer_profiles/qwen35_0p8b_attention_subset_cuda_third_pass.yaml")
    ...
    parser.add_argument("--hybrid-timeout-seconds", type=int, default=900)
...
def _run_jsonl_command(
    *,
    ...
    missing_record_factories: list[tuple[str, Any]],
) -> list[dict[str, Any]]:
    env = os.environ.copy()
...
def main() -> None:
    ...
    dense_path = output_dir / "qwen35_0p8b_dense_sweep.jsonl"
    statecache_path = output_dir / "qwen35_0p8b_statecache_sweep.jsonl"
    hybrid_path = output_dir / "qwen35_0p8b_hybrid_sweep.jsonl"
    turboquant_path = output_dir / "qwen35_0p8b_turboquant_sweep.jsonl"
    for path in (dense_path, statecache_path, hybrid_path, turboquant_path):
        if path.exists():
            path.unlink()
    ...
    statecache_command = [
        python_exe,
        str(REPO_ROOT / "benchmarks" / "bench_qwen35_deltanet_statecache_readout.py"),
        "--model-id",
        args.model_id,
        "--backend",
        args.backend,
    ...
    hybrid_command = [
        python_exe,
        str(REPO_ROOT / "benchmarks" / "bench_qwen35_attention_subset_statecache_dotcache.py"),
        "--model-id",
        args.model_id,
        "--backend",
        args.backend,
    ...
    _run_jsonl_command(
        ...
        timeout_seconds=args.hybrid_timeout_seconds,
        timeout_record_factory=lambda status="error", error_type="TimeoutExpired", error_message=None: {
            ...
            "status": status,
            "error_type": error_type,
            "error_message": error_message or f"timed out after {args.hybrid_timeout_seconds}s",
        },
        missing_record_factories=[
            (
                lambda record, context=context: int(record.get("prompt_length") or -1),
                lambda mode, context=context: context if mode == "key_only" else {
                    ...
                    "status": "error",
                    "error_type": "TimeoutExpired",
                    "error_message": f"timed out after {args.hybrid_timeout_seconds}s",
                },
            )
            for context in args.contexts
        ],
