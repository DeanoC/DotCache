...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an offline learned residual predictor on persistent replay compare outputs.")
    parser.add_argument("--compare-inputs", nargs="*", default=[])
    parser.add_argument("--compare-glob", default=None)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--abs-threshold", type=float, default=0.1)
...
def _render_markdown(summary: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "## Train",
        "",
        f"- accuracy: {float(train_metrics['accuracy']):.3f}",
        f"- precision: {float(train_metrics['precision']):.3f}",
        f"- recall: {float(train_metrics['recall']):.3f}",
        f"- f1: {float(train_metrics['f1']):.3f}",
        f"- safe prediction rate: {float(train_metrics['safe_prediction_rate']):.3f}",
        "",
        "## Test",
        "",
        f"- accuracy: {float(test_metrics['accuracy']):.3f}",
        f"- precision: {float(test_metrics['precision']):.3f}",
        f"- recall: {float(test_metrics['recall']):.3f}",
        f"- f1: {float(test_metrics['f1']):.3f}",
        f"- safe prediction rate: {float(test_metrics['safe_prediction_rate']):.3f}",
        f"- positives in test: {float(test_metrics['positive_rate']):.3f}",
        "",
        "## Features",
        "",
    ]
...
def main() -> int:
    ...
    train_metrics = evaluate_persistent_residual_predictor(model, split["train_records"])
    test_metrics = evaluate_persistent_residual_predictor(model, split["test_records"])

    output_dir = Path(args.output_dir).resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    artifact_path = output_dir / "persistent_residual_predictor_model.json"
    summary_path = output_dir / "persistent_residual_predictor_summary.json"
    markdown_path = output_dir / "persistent_residual_predictor_summary.md"
