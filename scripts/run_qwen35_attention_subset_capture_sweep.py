...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a small local Qwen3.5 attention-subset capture sweep and merge the resulting manifests.")
    ...
    parser.add_argument("--tokens-per-page", type=int, default=2)
...
def main() -> int:
    ...
    if not transformers_available():
        raise SystemExit("run_qwen35_attention_subset_capture_sweep.py requires the optional transformers dependencies")

    prompt_families = args.prompt_family or ["cache", "reasoning", "instruction", "retrieval"]
    prompt_lengths = args.prompt_length or [16, 32, 64]
    decode_steps = args.decode_steps or [1, 2, 4]
    ...
    merged_manifest = merge_page_trace_manifests(
        ...
        source="qwen35_attention_subset_capture_sweep",
    )
