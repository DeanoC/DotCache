...
/// Must match the C struct layout exactly (`#[repr(C)]`).
#[repr(C)]
#[derive(Debug, Clone)]
pub struct DecodeLayerDesc {
    pub layer_type: c_int,                  // 0=linear, 1=full
...
impl Default for DecodeLayerDesc {
    fn default() -> Self {
        // Safety: zero-initialized struct with null pointers is valid
...
impl MinimalQwen35Runner {
    ...
    fn build_direct_runtime(
        ...
        let full_attention_key_entry = metadata
            .workspace
            .iter()
            .find(|entry| entry.name == "full_attention_key")
            .ok_or_else(|| RuntimeError::External {
                context: "qwen35-hip-direct",
                message: "direct HIP metadata missing full_attention_key workspace".to_string(),
            })?;
        let full_attention_value_entry = metadata
            .workspace
            .iter()
            .find(|entry| entry.name == "full_attention_value")
            .ok_or_else(|| RuntimeError::External {
                context: "qwen35-hip-direct",
