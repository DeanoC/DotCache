...
from bench_common import build_config, build_queries, emit, parse_args
...
def main() -> None:
    args = parse_args(
        "Benchmark growing-context decode where only newly appended pages are prepared.",
        default_repeats=1,
    )
    ...
    for context_length in args.contexts:
        ...
        queries = build_queries(initial_context, config.head_dim, steps=args.decode_steps, seed=args.seed + 20_000)
        ...
        for step_index, query in enumerate(queries):
            ...
            mps_total_ms += _time_ms(
                lambda q=query, kp=list(key_pages), vp=list(value_pages), st=step_trace: mps_outputs.append(
                    decode_step(q, kp, vp, backend=args.backend, cache=cache, trace=st)[2]
                )
            )
        ...
        emit(
            {
                ...
                "mps_decode_ms_per_step": mps_total_ms / args.decode_steps,
                ...
                "step_host_to_device_bytes": growth_trace.host_to_device_bytes / args.decode_steps,
