...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DotCache replay benchmark for the Qwen3.5 full-attention subset.")
...
def _run_case(
    ...
    *,
    ...
    try:
        record = harness.run_attention_subset_dotcache(
            input_ids=input_ids,
        ...
        error_record.update(
            {
                "benchmark": "qwen35_attention_subset_dotcache",
                "status": "error",
...
def main() -> None:
    ...
    if not transformers_available():
        raise SystemExit("bench_qwen35_attention_subset_dotcache.py requires the optional transformers dependencies")
