...
@triton.jit
def _hybrid_mixedv_deqk_split_k_partial_kernel(
    K_deq_fp16_ptr, K_fp16_ptr,
    TopK_mask_ptr,
    V_packed_ptr, V_scales_ptr, V_zeros_ptr,
    V_fp16_ptr, V_fp16_mask_ptr, V_block_slots_ptr,
    Q_ptr, Skip_ptr,
    ...
    """Split-K mixed-value kernel using persistent FP16 dequantized tail keys."""
    prog = tl.program_id(0)
    qh = prog // num_splits
    sp = prog % num_splits
    valid_q = qh < num_q_heads
    if valid_q:
        kvh = qh // gqa_group
        kdeq_base_elem = kvh * stride_kv_kdeq
        kfp16_base_elem = kvh * stride_kv_kfp16
        vpack_base_elem = kvh * stride_kv_vpack
        vscale_base_elem = kvh * stride_kv_vscale
        vscratch_base_elem = kvh * stride_kv_vscratch
        ...
        for bid in range(block_start, block_end):
            ...
            if skip_val == 0:
                ...
                deq_row_ptrs = K_deq_fp16_ptr + kdeq_base_elem + (base_tok + t_offs) * stride_k
                fp16_row_ptrs = K_fp16_ptr + kfp16_base_elem + (base_tok + t_offs) * stride_k
                scores = tl.zeros((block_size,), dtype=tl.float32)
                ...
                for d_start in range(0, head_dim, TILE_D):
                    ...
                    k_tile = tl.where(use_fp16_key == 1, k_fp16, k_deq)
                    scores += tl.sum(k_tile * q_tile[None, :], axis=1)
                scores = scores * q_scale
...
def selective_attend_multihead_hybrid_mixedv_deqk_split_k(
    keys_deq_fp16: torch.Tensor,
    keys_fp16: torch.Tensor,
    topk_mask: torch.Tensor,
    values_int4_packed: torch.Tensor,
    values_int4_scales: torch.Tensor,
    ...
    """Split-K mixed-value attend with persistent FP16 dequantized tail keys.

    Semantics match `selective_attend_multihead_hybrid_mixedv_split_k`: blocks
    marked in `topk_mask` still use exact FP16 keys, while tail blocks use the
    asymmetric INT8 reconstruction stored in `keys_deq_fp16`.
    """
    num_kv_heads, N, head_dim = keys_deq_fp16.shape
    d_v = values_int4_packed.shape[2] * 2
    num_groups = d_v // group_size
    num_q_heads = q_all.shape[0]
    num_blocks = N // block_size
    device = keys_deq_fp16.device

    if num_splits is None:
        target_blocks_per_split = int(os.environ.get("DOTCACHE_MIXEDV_DEQK_SPLITK_BLOCKS_PER_SPLIT", "64"))
        ns = max(1, (num_blocks + target_blocks_per_split - 1) // target_blocks_per_split)
        num_splits = 1
        while num_splits < ns:
            num_splits *= 2
    ...
    assert keys_deq_fp16.stride(2) == 1 and keys_deq_fp16.stride(1) == head_dim
    assert keys_fp16.stride(2) == 1 and keys_fp16.stride(1) == head_dim
    assert values_int4_packed.stride(2) == 1 and values_int4_packed.stride(1) == d_v // 2
    assert values_int4_scales.stride(2) == 1 and values_int4_scales.stride(1) == num_groups
    assert values_int4_zeros.stride(2) == 1 and values_int4_zeros.stride(1) == num_groups
    ...
    stride_kv_kdeq = keys_deq_fp16.stride(0)
    stride_kv_kfp16 = keys_fp16.stride(0)
    stride_kv_vpack = values_int4_packed.stride(0)
    stride_kv_vscale = values_int4_scales.stride(0)
    stride_kv_vscratch = values_fp16_scratch.stride(0)
    ...
    _hybrid_mixedv_deqk_split_k_partial_kernel[(num_q_heads * num_splits,)](
        keys_deq_fp16,
        keys_fp16,
        topk_mask.contiguous(),
        values_int4_packed,
