...
# Calibrated profile for LLaMA 3.1-8B (cos ≥ 0.999 target).
# Generated by benchmarks/calibrate_epsilon_profile.py.
# Layers 29/31 hit cos=0.000 at some contexts even at ε=1e-7 — these are
# set to 1e-7 (effectively disabling skip for those layers).
LLAMA_31_8B_PROFILE = {
    4096: {
        **{lid: 1e-3 for lid in range(0, 28)},
        28: 5e-4,
        **{lid: 1e-7 for lid in range(29, 32)},
    },
    8192: {
        **{lid: 1e-3 for lid in range(0, 7)},
        **{lid: 5e-4 for lid in range(7, 29)},
        **{lid: 1e-4 for lid in range(29, 31)},
        31: 1e-7,
    },
    16384: {
        **{lid: 5e-4 for lid in range(0, 8)},
        8: 1e-4,
        9: 5e-4,
        **{lid: 1e-4 for lid in range(10, 29)},
        29: 1e-7,
        30: 1e-5,
        31: 1e-7,
    },
    32768: {
        **{lid: 1e-4 for lid in range(0, 28)},
        28: 5e-5,
        29: 1e-5,
        30: 1e-4,
        31: 1e-7,
    ...
    65536: {
        # 64K: conservative — use 32K profile until calibrated
        **{lid: 1e-4 for lid in range(0, 28)},
        28: 5e-5,
        29: 1e-5,
        30: 1e-4,
