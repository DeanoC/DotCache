...
- `Qwen3.5 hybrid`
  ...
  - There is now a separate StateCache probe lane for the `linear_attention` / DeltaNet side:
    - DeltaNet state inspection
    - dense-only pre/post/readout ablations
    - synthetic `M0` / `M3` state-tile simulator
...
- The next Qwen3.5 step is no longer only attention-side tuning. The repo now has a separate StateCache probe lane for DeltaNet state so the CUDA path can compare attention-subset DotCache and recurrent-state exploration in parallel.
